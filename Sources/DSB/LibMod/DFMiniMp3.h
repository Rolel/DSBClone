...
public:
    explicit DFMiniMp3(T_SERIAL_METHOD& serial) :
        _serial(serial),
        _lastSendSpace(offset_msSendSpace),
        _isOnline(false)
    {
    }
...
// Added to manage on the fly spacing
    uint16_t increaseSendSpace()
    {
        if (offset_msSendSpace < 1500) { offset_msSendSpace += 10; }
        return offset_msSendSpace;
    }

    uint16_t decreaseSendSpace()
    {
        offset_msSendSpace -= 10;
        if (offset_msSendSpace > 2000) { offset_msSendSpace = 0; }
        return offset_msSendSpace;
    }

    uint16_t getSendSpace()
    {
        return offset_msSendSpace;
    }

    uint16_t setSendSpace(uint16_t offset)
    {
        return offset_msSendSpace = offset;
    }
    

private:
// Reduce delay to something very low: 10
// Default is 50, some bad clone chip need 150
    uint16_t offset_msSendSpace = 10;
    ...
    void sendPacket(uint8_t command, uint16_t arg = 0, uint16_t sendSpaceNeeded = 0)
    {
        typename T_CHIP_VARIANT::SendPacket packet = T_CHIP_VARIANT::generatePacket(command, arg);

        sendSpaceNeeded += offset_msSendSpace;
